...
void us_internal_loop_data_init(struct us_loop *loop, void (*wakeup_cb)(struct us_loop *loop),
                                void (*pre_cb)(struct us_loop *loop), void (*post_cb)(struct us_loop *loop)) {
...
void us_internal_dispatch_ready_poll(struct us_poll *p, int error, int events) {
  ...
  switch (us_internal_poll_type(p)) {
    case POLL_TYPE_CALLBACK: {
      ...
      cb->cb(
          cb->cb_expects_the_loop ? (struct us_internal_callback *) cb->loop : (struct us_internal_callback *) &cb->p);
    }
      break;
    case POLL_TYPE_SEMI_SOCKET: {
      ...
      if (us_poll_events(p) == LIBUS_SOCKET_WRITABLE) {
        ...
        if (client_fd == LIBUS_SOCKET_ERROR) {
          ...
          // TODO: 处理accept的失败的方法
          do {
            struct us_poll *p = us_create_poll(us_socket_get_context(&listen_socket->s)->loop, 0,
                                               sizeof(struct us_socket) - sizeof(struct us_poll) +
    ...
    case POLL_TYPE_SOCKET: {
      ...
      if (events & LIBUS_SOCKET_READABLE) {
        ...
        int length = bsd_recv(us_poll_fd(&s->p), s->context->loop->data.recv_buf + LIBUS_RECV_BUFFER_PADDING,
